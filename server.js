...
const IDler = {
  botID: "752228343478354051",
  botSecret: process.env.botSecret,
  botCallbackURL: process.env.callback,
  sunucuID: "884441353356996649",
  sunucuDavet: "https://discord.gg/ghchCGYEUC",
  kodLogKanalı: "933035369862148166",
  sahipRolü: "932263354708418560",
  adminRolü: "932263355157205013",
  kodPaylaşımcıRolü: "933035610300641380",
  boosterRolü: "884453130467688448", 
  kodPaylaşamayacakRoller: ["932263355765370910", "926381966918570014"],
  hazırAltyapılarRolü: "933035986731028511",
  hazırSistemlerRolü: "933035986731028511",
  elmasKodlarRolü: "933036050757091348",
  altınKodlarRolü: "933036106629382144",
  normalKodlarRolü: "932263355765370910"
};
  ...
  if (code) {
    ...
    if (
      ...
      (member.roles.cache.has(IDler.altınKodlarRolü) ||
        ...
        member.roles.cache.has(IDler.jsRolü) ||
...
app.get("/topluluk/:id", (req, res) => {
  ...
  if (code) {
    ...
    if (
      member &&
      (member.roles.cache.has(IDler.altyapılarRolü) ||
        member.roles.cache.has(IDler.boosterRolü) ||
        member.roles.cache.has(IDler.sahipRolü) ||
        member.roles.cache.has(IDler.kodPaylaşımcıRolü) ||
        member.roles.cache.has(IDler.adminRolü))
  ...
  if (!member)
    ...
    let perms = {
      ...
      hazir:
        ...
        member.roles.cache.has(IDler.kodPaylaşımcıRolü) ||
        member.roles.cache.has(IDler.boosterRolü) ||
        member.roles.cache.has(IDler.hazırRolü) ||
        member.roles.cache.has(IDler.AltyapılarRolü),
...
app.post("/paylasim", (req, res) => {
  ...
  client.channels.cache.get(IDler.kodLogKanalı).send(
    ...
    .addField("Kod Sayfası", `[Tıkla!](https://code-makare.glitch.me/${obj.kod_rank}/${obj.id})`));
...
client.on("ready", () => {
  const listener = app.listen(process.env.PORT, function() {
    client.user.setActivity(`CoeMakare Sunucusuna Code Ekliyor`, {  })
...
client.on("ready", async () => {
  ...
  let botVoiceChannel = client.channels.cache.get("932263363717779536");
